...
public class UplataService {
    ...
    public Uplata createUplata(UplataModel model) {
        Clan clan = clanoviRepository.findByIdAndDeletedAtIsNull(model.getClan().getId()).orElseThrow();
        Uplata uplata = new Uplata();
        uplata.setClan(clan);
        uplata.setIznos(model.getIznos());
        uplata.setDatumUplate(model.getDatumUplate()!= null ? model.getDatumUplate() : LocalDateTime.now());
